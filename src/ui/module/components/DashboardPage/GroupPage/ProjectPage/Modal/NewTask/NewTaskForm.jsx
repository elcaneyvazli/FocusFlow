...
import { Activity, Bookmark, LayoutDashboard, Users } from "lucide-react";
...
export default function NewTaskForm({ onClose, groupId, projectId, projectUsers = [], mutate }) {
  ...
  console.log("NewTaskForm - Initial props:", { groupId, projectId });

  if (!groupId || !projectId) {
    console.error("NewTaskForm - Missing required IDs:", {
      groupId,
      projectId,
    });
    dispatch(
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!data.taskTitle || !data.taskDescription || !data.taskLabel) {
      ...
      const taskData = {
        ...
        label: data.taskLabel.trim(),
        dueDate: selectedDate.toISOString(),
        priority: priorityMap[columnValue] || 0,
        usernamesOrEmails: selectedUsers.length > 0 ? selectedUsers : [], // Ensure it's always an array
        status: statusMap[activityValue] || 0,
      };

      // Debug log
      console.log("Submitting task data:", taskData);

      const response = await createProjectTask(taskData, mutate, groupId, projectId);
    ...
    >
      <div className="flex flex-col gap-16 px-16 py-16 ">
        ...
        <div className="flex flex-col md:flex-row justify-center gap-16">
          ...
          />
          <SelectInput
            data={projectUsers?.map(user => user.username) || []}
            value={selectedUsers}
            setValue={setSelectedUsers}
            inputEnabled={true}
            multiple={true}
            label="Assign Users"
            icon={<Users className="text-light" size={18} />}
            size="medium"
            type="base"
          />
        </div>
