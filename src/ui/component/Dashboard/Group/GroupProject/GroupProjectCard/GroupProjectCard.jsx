...
export default function GroupProjectCard({ project }) {
  const router = useRouter();
  return (
    <motion.div
      className="text-primary dark:text-input-bg col-span-12 lg:col-span-6 bg-white dark:bg-dark-input-bg border border-input-border dark:border-dark-input-border rounded-main p-12 flex flex-col gap-8 cursor-pointer"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.3 }}
      whileHover={{ scale: 1.05 }}
      whileTap={{ scale: 0.95 }}
      onClick={() => router.push(`/dashboard/group/${project.id}`)}
    >
